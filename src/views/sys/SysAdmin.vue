<template>
  <div>
    <div style="display: flex;justify-content: center;margin-top: 10px">
      <el-input v-model="keywords" aria-placeholder="通过用户名搜素用户..." prefix-icon="el-icon-search" style="width: 400px;margin-right: 10px"></el-input>
      <el-button icon="el-icon-search" type="primary" @click="doSearch">搜素</el-button>
    </div>
    <div class="admin-container">
      <el-card class="admin-card" v-for="(admin,index) in admins" :key="index">
        ...
        <div class="userinfo-container">
          ...
          <div>座机号码:{{admin.telephone}}</div>
          <div>住址:{{admin.address}}</div>
          <div>用户状态:
            <el-switch
          ...
          <div>
            用户角色:<el-tag style="margin-right: 10px" type="success" v-for="(role,indexj) in admin.roles" :key="indexj">{{role.nameZh}}</el-tag>
            <el-popover
                placement="right"
                title="角色列表"
                width="200"
...
export default {
  ...
  methods:{
    ...
    initAdmins(){
      this.getRequest(`/chat/admin/?keywords=`+this.keywords).then(resp=>{
        if (resp){
          this.admins=resp;
        }
      })
...
<style>
  ...
  .userinfo-container div:nth-child(-n+6){
    margin-top: 5px;
  }
</style>