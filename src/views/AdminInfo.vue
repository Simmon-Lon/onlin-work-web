    ...
    <el-card class="box-card" style="width: 400px">
      ...
      <div>
        <div style="display: flex;justify-content: center">
          <img title="点击用户头像" :src="admin.userFace" style="height: 100px;width: 100px;border-radius: 50px" alt=""/>
        </div>
        <div style="margin: 5px 5px">
          电话号码:
          <el-tag type="success">{{admin.telephone}}</el-tag>
        </div>
        <div>
          手机号码:
        ...
        <div style="margin: 5px 5px">
          地址:
          <el-tag type="success">{{admin.address}}</el-tag>
        </div>
        <div>
          用户标签:
          <el-tag type="success" v-for="(r,index) in admin.roles" :key="index">{{r.nameZh}}</el-tag>
        </div>
        <div style="display: flex;justify-content: space-around;margin: 5px 5px">
  ...
  methods:{
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.resetForm.adminId =this.admin.id;
